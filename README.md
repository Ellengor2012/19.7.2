...
def test_add_new_pet_without_photo(name='Тузик', animal_type='собака', age=5):
    """Проверяем что можно добавить информацию о новом питомце без фото"""

    # Запрашиваем ключ api и сохраняем в переменую auth_key
    _, auth_key = pf.get_api_key(valid_email, valid_password)

    # Добавляем информацию о новом питомце без фото
...
def test_add_photo_of_pet(pet_photo='nemetskaya-ovcharka8.jpg'):
